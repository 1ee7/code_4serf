...
#include <string.h> 
#include <fcntl.h>
#include "./lib/mavlink-custom/1.0/tx1control/mavlink.h"
#include "libvisca.h"
...
// printf("tgl debug \n");
  switch(p_msg->msgid)
  {
   ...
   case MAVLINK_MSG_ID_DIGICAM_CONTROL:
        mavlink_msg_digicam_control_decode(p_msg, &digicam_control);
        value=digicam_control.extra_param;
        mode= value & 0x60 ;
        printf("the value is %x ,mode is %x\n",value,mode); 
        printf("the value & 0x80 is %x\n",value&0x80);
        if( value == 0x00) //stop zoom
        {
            if (VISCA_set_zoom_stop(&iface, &camera)!=VISCA_SUCCESS)
               fprintf(stderr,"error stop zoom \n");
            else
               fprintf(stderr,"stop zoom\n");
        }

       else 
       {
         if (mode == 0x00)  //nomal zoom in
         {
           if (VISCA_set_zoom_tele(&iface, &camera)!=VISCA_SUCCESS)
             fprintf(stderr,"error getting zoom in \n");
          else
            fprintf(stderr,"Zoom in done\n");
           /* code */
         }
         else if (mode == 0x20)
         {
          if (VISCA_set_zoom_wide(&iface, &camera)!=VISCA_SUCCESS)
             fprintf(stderr,"error getting zoom out\n");
          else
            fprintf(stderr,"Zoom out done\n");
         }
       }
...
/* init visca camera */
  ...
  if (VISCA_open_serial(&iface, "/dev/ttyS0")!=VISCA_SUCCESS)
    {
      fprintf(stderr," unable to open serial device /dev/ttyS0\n");
      exit(1);
    }
   /*  iface.broadcast=0;
